...
const DICT = {
  ...
  'playground.text': {
    en: 'Paint in the grid, or type the bitmap in the box: one row per line, a dot is an empty pixel and any other character is a painted one. Give each character a color, a hex value or a `var(--token)` from this page. You can also drop a PNG on the grid. The page address follows your drawing, so copying the link shares it.',
    es: 'Pinta en la cuadrícula, o escribe el bitmap en la caja: una fila por línea, un punto es un píxel vacío y cualquier otro carácter es uno pintado. Dale un color a cada carácter, un valor hex o un `var(--token)` de esta página. También puedes soltar un PNG sobre la cuadrícula. La dirección de la página sigue a tu dibujo, así que copiar el enlace lo comparte.',
  },
  'playground.editor': { en: 'Grid', es: 'Cuadrícula' },
  'playground.paintWith': { en: "Paint with '{ch}'", es: "Pintar con '{ch}'" },
  'playground.brush': { en: 'Brush', es: 'Pincel' },
  'playground.eraser': { en: 'Eraser', es: 'Goma' },
  'playground.undo': { en: 'Undo', es: 'Deshacer' },
  'playground.redo': { en: 'Redo', es: 'Rehacer' },
  'playground.clear': { en: 'Clear', es: 'Limpiar' },
  'playground.width': { en: 'Width', es: 'Ancho' },
  'playground.height': { en: 'Height', es: 'Alto' },
  'playground.addColor': { en: 'Add color', es: 'Añadir color' },
  'playground.import': { en: 'Import PNG', es: 'Importar PNG' },
  'playground.export': { en: 'Export PNG', es: 'Exportar PNG' },
  'playground.dropHint': {
    en: 'Drop an image on the grid, up to 64x64. Colors come in as hex; rename them to `var(--token)` in the palette if you want them to follow a theme.',
    es: 'Suelta una imagen sobre la cuadrícula, hasta 64x64. Los colores entran en hex; renómbralos a `var(--token)` en la paleta si quieres que sigan un tema.',
  },
  'playground.imported.big': {
    en: 'Imported at {w}x{h}. The playground is meant for sprites up to {max}x{max}; a drawing this size may not fit in a link.',
    es: 'Importado a {w}x{h}. El playground está pensado para sprites de hasta {max}x{max}; un dibujo de este tamaño puede no caber en un enlace.',
  },
  'playground.import.size': {
    en: 'The image is {w}x{h}; the limit is {max}x{max}.',
    es: 'La imagen mide {w}x{h}; el límite es {max}x{max}.',
  },
  'playground.import.colors': {
    en: 'The image has {n} colors; the maximum is {max}. Reduce them and try again.',
    es: 'La imagen tiene {n} colores; el máximo es {max}. Redúcelos y vuelve a intentarlo.',
  },
  'playground.import.read': { en: 'That file could not be read as an image.', es: 'No se pudo leer ese archivo como imagen.' },
  'playground.tooBigToShare': { en: 'Too big for a link', es: 'Demasiado grande para un enlace' },
  'playground.tools': { en: 'Tools', es: 'Herramientas' },
  'playground.textTab': { en: 'Text', es: 'Texto' },
  'playground.noColor': { en: 'No color yet', es: 'Sin color todavía' },
  'playground.removeColor': { en: "Remove '{ch}' and erase its pixels", es: "Quitar '{ch}' y borrar sus píxeles" },
